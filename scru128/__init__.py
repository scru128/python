...
class Scru128Generator:
    ...
    def generate(self) -> Scru128Id:
        ...
        is detected, this method resets the generator state and returns a new ID based
    ...
    def generate_monotonic(self) -> typing.Optional[Scru128Id]:
        """
        Generates a new SCRU128 ID object from the current `timestamp`, guaranteeing
        that the returned ID is greater than the immediately preceding one generated by
        the same generator.
        ...
        with self._lock:
            timestamp = datetime.datetime.now().timestamp()
            return self.generate_core_monotonic(int(timestamp * 1_000))
    ...
    def generate_core(self, timestamp: int) -> Scru128Id:
        ...
        method resets the generator state and returns a new ID based on the given
        argument.
        ...
        """
        value = self.generate_core_monotonic(timestamp)
        if value is None:
    ...
    def generate_core_monotonic(self, timestamp: int) -> typing.Optional[Scru128Id]:
        """
        Generates a new SCRU128 ID object from the `timestamp` passed, guaranteeing that
        the returned ID is greater than the immediately preceding one generated by the
        same generator.
        ...
        Unlike `generate_monotonic()`, this method is NOT thread-safe. The generator
